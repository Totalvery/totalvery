...
class DoordashCrawler:
    ...
    def get_store(self, restaurantId):
        ...
        with open('doordash_detail.json', mode='w') as f:
            f.write(json.dumps(store_json, indent=2))
...
class GrubhubCrawler:
    ...
    def get_store(self, restaurantId):
        ...
        offer = self.s.get(
            f'https://api-gtm.grubhub.com/offers/{restaurantId}?orderType=STANDARD&offerCategoryType=ALL')

        with open('grubhub_detail.json', mode='w') as f:
            f.write(json.dumps(store_json, indent=2))
        return store_json, offer.json()
